...
def dual_benchmark_precision_check(
    ...
    if not inf_nan_match:
        report = DualBenchmarkReport(
            passed=False, level=level,
            mare_npu=0, mere_npu=0, rmse_npu=0,
            mare_ref=0, mere_ref=0, rmse_ref=0,
            mare_ratio=0, mere_ratio=0, rmse_ratio=0,
            mare_th=mare_th, mere_th=mere_th, rmse_th=rmse_th,
            mare_passed=False, mere_passed=False, rmse_passed=False,
            small_value_error_count_npu=0, small_value_error_count_ref=0,
            small_value_passed=False, inf_nan_match=False,
            num_elements=num_elements, message="INF/NAN mismatch detected",
        )
        if verbose:
            print(report.summary())
        return report
    ...
    final_passed = (
        mare_passed and mere_passed and rmse_passed
        and small_value_passed and inf_nan_match
    )
    ...
    report = DualBenchmarkReport(
        passed=final_passed, level=level,
        mare_npu=mare_npu, mere_npu=mere_npu, rmse_npu=rmse_npu,
        mare_ref=mare_ref, mere_ref=mere_ref, rmse_ref=rmse_ref,
        mare_ratio=mare_ratio, mere_ratio=mere_ratio, rmse_ratio=rmse_ratio,
        mare_th=mare_th, mere_th=mere_th, rmse_th=rmse_th,
        mare_passed=mare_passed, mere_passed=mere_passed, rmse_passed=rmse_passed,
        small_value_error_count_npu=ec_npu, small_value_error_count_ref=ec_ref,
        small_value_passed=small_value_passed, inf_nan_match=inf_nan_match,
        num_elements=num_elements, message=message,
    )
    if verbose:
        print(report.summary())
    return report
...
def bootstrap_retest(
    ...
    if verbose:
        ...
        for name, r in results.items():
            ...
            print(
                f"  {name}: median={r['median']:.6f}  "
                f"95% CI=[{r['ci_lower']:.6f}, {r['ci_upper']:.6f}]  {status}"
            )
