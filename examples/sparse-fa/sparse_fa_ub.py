...
def high_perf_mtgr_sparse_attn_kernel(
    ...
    def main(
        ...
        with T.Kernel(core_num, is_npu=True) as (cid, vid):
            ...
            # segment_offsets_len = T.alloc_var("int32", init=0)
            segment_offsets_len = T.ceildiv((max_segs + 1) * 4, UB_BLOCK_LEN ) * UB_BLOCK_LEN 
            segment_offsets_ub = T.alloc_ub([max_segs + 1], "int32")
            segment_rules_ub = T.alloc_ub([max_segs], "int32")

            # segment_offsets_ub = T.alloc_ub([1024 + 1], "int32")
            # segment_rules_ub = T.alloc_ub([1024], "int32")
            ...
            T.annotate_address(
                {
                    ...
                    row_rule_buf: half_M * dim * 4 + num_stages * half_M * 4 * 2 + half_M * 12 + half_M * block_N * 8 + half_M * block_N * 4,
                    row_seg_start_buf: half_M * dim * 4 + num_stages * half_M * 4 * 2 + half_M * 12 + half_M * block_N * 8 + half_M * block_N * 4 + half_M * 4,
                    row_seg_end_buf: half_M * dim * 4 + num_stages * half_M * 4 * 2 + half_M * 12 + half_M * block_N * 8 + half_M * block_N * 4 + half_M * 4 * 2,
                    segment_offsets_ub: half_M * dim * 4 + num_stages * half_M * 4 * 2 + half_M * 12 + half_M * block_N * 8 + half_M * block_N * 4 + half_M * 4 * 3,
                    segment_rules_ub: half_M * dim * 4 + num_stages * half_M * 4 * 2 + half_M * 12 + half_M * block_N * 8 + half_M * block_N * 4 + half_M * 4 * 3 + segment_offsets_len,
                    # segment_rules_ub: half_M * dim * 4 + num_stages * half_M * 4 * 2 + half_M * 12 + half_M * block_N * 8 + half_M * block_N * 4 + half_M * 4 * 3 + 4128,
                }
            )
            ...
            with T.Scope("C"):
                ...
                for core_index in T.serial(my_iters):
                    ...
                    gap_size = T.if_then_else(
                        (q_start >= last_seg_start) & (gap_k_end > gap_k_start),
                        gap_k_end - gap_k_start, 0)
            ...
            with T.Scope("V"):
                ...
                # T.copy(segment_rules, segment_rules_ub[0:max_segs])
                ...
                for core_index in T.serial(my_iters):
                    ...
                    T.copy(segment_offsets[b_i, :], segment_offsets_ub[0:(max_segs+1)])
                    T.set_flag("mte2", "s", 5)
                    T.wait_flag("mte2", "s", 5)
                    total_seq_len_b = segment_offsets_ub[max_segs]
                    q_start = s_local * block_M
                    q_end = T.if_then_else(q_start + block_M < total_seq_len_b, q_start + block_M, total_seq_len_b)
                    ...
                    _rule1_start = segment_offsets_ub[rule1_seg_idx]
                    _rule1_end = segment_offsets_ub[rule1_seg_idx + 1]
                    _overlaps = (_rule1_end > q_start) & (_rule1_start < q_end)
                    k_upper_bound = T.if_then_else(_overlaps & (_rule1_end > q_end), _rule1_end, q_end)
                    num_k_tiles = T.ceildiv(k_upper_bound, block_M)

                    last_seg_start = segment_offsets_ub[max_segs - 1]
                    gap_k_start = T.ceildiv(last_seg_start, block_M)
                    gap_k_end = q_start // block_M
                    gap_size = T.if_then_else(
                        (q_start >= last_seg_start) & (gap_k_end > gap_k_start),
                    ...
                    for row in T.serial(half_M):
                        ...
                        for _bi in T.serial(bin_iters):
                            _mid = (_lo + _hi) // 2
                            _active = _lo < _hi
                            _le = segment_offsets_ub[_mid] <= row_abs_pos
                            _gt = segment_offsets_ub[_mid] > row_abs_pos
                            _lo = T.if_then_else(_active & _le, _mid + 1, _lo)
                            _hi = T.if_then_else(_active & _gt, _mid, _hi)
                        seg_id = _lo - 1

                        # row_rule_buf[row] = segment_rules_ub[seg_id]
                        row_rule_buf[row] = segment_rules[seg_id]
                        row_seg_start_buf[row] = segment_offsets_ub[seg_id]
                        row_seg_end_buf[row] = segment_offsets_ub[seg_id + 1]
...
def high_perf_sparse_attn_wrapper(
    ...
    ws1 = torch.zeros((core_num, num_stages, block_M, block_N), dtype=torch.bfloat16, device=query.device)
    ws2 = torch.zeros((core_num, num_stages, block_M, block_N), dtype=torch.bfloat16, device=query.device)
    ws3 = torch.zeros((core_num, num_stages, block_M, D), dtype=torch.bfloat16, device=query.device)
    output = torch.empty_like(query)

    bin_iters = max(1, math.ceil(math.log2(max_segs + 1)))
