...
def golden_attention_float64(
    ...
    per_batch, kv_group = _prepare_kv_and_mask(
        query_snd, key_snd, value_snd,
        segment_offsets_i32, segment_rules_i32,
        q_seq_starts_i32, matched_prefix_lens_i32,
        key_cache, value_cache, block_table_i32, block_size,
    )
    ...
    for item in per_batch:
        ...
        ref_b = torch.einsum("hqk,khd->qhd", probs, v).to(torch.bfloat16)
...
def golden_attention_simulated_kernel(
    ...
    for item in per_batch:
        q = item["q_b"].float()
        k = item["k_full_b"].float()
        v = item["v_full_b"].float()
        full_mask = item["full_mask"]

        S_raw = torch.einsum("qhd,khd->hqk", q, k)
        S_ws1 = S_raw.to(torch.bfloat16).float()

        S_scaled = S_ws1 * sm_scale
        S_scaled = S_scaled.masked_fill(full_mask.unsqueeze(0) == 0.0, float("-inf"))

        m = torch.max(S_scaled, dim=-1, keepdim=True).values
        P_unnorm = torch.exp(S_scaled - m).nan_to_num(0.0)

        P_ws2 = P_unnorm.to(torch.bfloat16).float()

        sumexp = torch.sum(P_unnorm, dim=-1, keepdim=True)

        O_unnorm = torch.einsum("hqk,khd->qhd", P_ws2, v)
        O_ws3 = O_unnorm.to(torch.bfloat16).float()

        sumexp_q = sumexp.permute(1, 0, 2)

        out = O_ws3 / sumexp_q.clamp(min=1.0)
        ref_outputs.append(out.to(torch.bfloat16))
